...
const App: React.FC = () => {
  ...
  // Recurrence & Installment states for forms
  ...
  // States for dynamic category creation (Revenue)
  const [isAddingRevCategory, setIsAddingRevCategory] = useState(false);
  const [newRevCategoryName, setNewRevCategoryName] = useState('');

  // States for dynamic category creation (Expense)
  const [isAddingExpCategory, setIsAddingExpCategory] = useState(false);
  ...
  const handleAddRevCategory = () => {
    if (newRevCategoryName.trim()) {
      addCategory(newRevCategoryName, TransactionType.REVENUE);
      setNewRevCategoryName('');
      setIsAddingRevCategory(false);
    }
  };

  const handleAddExpCategory = () => {
    if (newExpCategoryName.trim()) {
      addCategory(newExpCategoryName, TransactionType.EXPENSE);
      setNewExpCategoryName('');
      setIsAddingExpCategory(false);
    }
  };
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center justify-center text-center gap-6">
               ...
               <div>
                  <h3 className="text-xl font-bold text-slate-800 mb-2 uppercase">Controle Total</h3>
                  <p className="text-sm text-slate-500 uppercase">Mantenha seus registros em dia para uma vida financeira saudável.</p>
               </div>
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-50 uppercase text-[13px]">
                {filteredData.monthlyRevenues.map(r => (
                  <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <button onClick={() => toggleRevenueStatus(r.id)} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase transition-colors ${r.status === Status.PAID ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{r.status === Status.PAID ? 'Recebida' : 'Pendente'}</button>
                    </td>
                ...
                {filteredData.monthlyExpenses.map(e => (
                  <tr key={e.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <button onClick={() => toggleExpenseStatus(e.id)} className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase transition-colors ${e.status === Status.PAID ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700'}`}>{e.status === Status.PAID ? 'Paga' : 'Pendente'}</button>
                    </td>
                    <td className="px-6 py-4 text-right">
                ...
                {filteredData.monthlyInstallments.map(i => {
                   ...
                   return (
                    <tr key={i.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right"><span className="text-slate-300">🔒</span></td>
                    </tr>
                   )
                })}
              </tbody>
      ...
      <div className="p-6 md:p-8 space-y-6 animate-in slide-in-from-right-4 duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {debts.map(debt => {
            ...
            return (
              <div key={debt.id} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col gap-4 group hover:border-indigo-200 transition-colors uppercase">
                <div className="flex justify-between items-start">
                  ...
                  <button onClick={() => setConfirmDeleteState({ id: debt.id, type: 'debt' })} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-rose-500 transition-all">🗑️</button>
                </div>
                <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                   <div className="h-full bg-indigo-500 transition-all duration-1000" style={{ width: `${progress}%` }}></div>
                </div>
                <div className="flex items-center justify-between pt-2 border-t border-slate-50 mt-2">
                  <div className="flex flex-col"><span className="text-lg font-bold text-indigo-600">{formatCurrency(debt.totalValue)}</span></div>
                  <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase ${debt.status === 'ACTIVE' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'}`}>{debt.status === 'ACTIVE' ? 'Ativo' : 'Finalizado'}</span>
                </div>
              </div>
            );
          })}
  ...
  const VisaoGeralHeaderContent = (
    <div className="bg-[#2196F3] text-white p-4 md:px-8 flex items-center justify-between uppercase">
      ...
      <div className="flex items-center gap-4"><button className="text-2xl hover:bg-white/10 w-10 h-10 flex items-center justify-center rounded-full transition-colors">⋮</button></div>
    </div>
  );
    ...
    <Layout activeTab={activeTab} setActiveTab={setActiveTab} headerContent={activeTab === 'dashboard' ? VisaoGeralHeaderContent : undefined} headerClassName={activeTab === 'dashboard' ? 'bg-[#2196F3]' : undefined}>
      ...
      <Modal isOpen={showRevForm} onClose={() => setShowRevForm(false)} title="NOVA RECEITA">
        <form onSubmit={(e) => {
          ...
          <div>
            ...
            <input name="desc" type="text" required className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none uppercase" placeholder="EX: SALÁRIO" />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Valor</label>
            <input name="value" type="number" step="0.01" required className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none uppercase" placeholder="0,00" />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Data</label>
            <input name="date" type="date" required defaultValue={todayStr} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none uppercase" />
          </div>

          <div className="flex items-center justify-between p-2 bg-slate-50 rounded-xl border border-slate-100">
            <span className="text-xs font-bold text-slate-600 uppercase">RECORRENTE</span>
            <Toggle 
              enabled={isRevRecurrent} 
              onChange={setIsRevRecurrent} 
          ...
          {isRevRecurrent && (
            <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2">
              <div>
                ...
                <select name="freq" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none uppercase">
                  <option value={Frequency.DAILY}>DIÁRIO</option>
                  <option value={Frequency.WEEKLY}>SEMANAL</option>
                  <option value={Frequency.MONTHLY} selected>MENSAL</option>
              ...
              <div>
                ...
                <input name="reps" type="number" defaultValue="12" min="2" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none uppercase" />
              </div>
            </div>
          )}
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-1">
              ...
              <button 
                type="button" 
                onClick={() => setIsAddingRevCategory(!isAddingRevCategory)}
            ...
            {isAddingRevCategory ? (
              <div className="flex gap-2 animate-in slide-in-from-top-2">
                <input 
                  autoFocus
                  className="flex-1 bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none uppercase" 
                  placeholder="NOVA CATEGORIA"
                  value={newRevCategoryName}
                  onChange={(e) => setNewRevCategoryName(e.target.value)}
                />
                <button 
                  type="button" 
                  onClick={handleAddRevCategory}
                  className="bg-emerald-600 text-white px-4 rounded-xl font-bold hover:bg-emerald-700"
                >
                  OK
                </button>
              </div>
            ) : (
              <select name="category" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none uppercase">
          ...
          <button type="submit" className="w-full bg-emerald-600 text-white p-3 rounded-xl font-bold shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all uppercase">Salvar</button>
        </form>
      </Modal>
      ...
      <Modal isOpen={showExpForm} onClose={() => setShowExpForm(false)} title="NOVA DESPESA">
        <form onSubmit={(e) => {
          ...
          const description = String(fd.get('desc')).toUpperCase();
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Descrição</label>
            <input name="desc" type="text" required className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none uppercase" placeholder="EX: MERCADO" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex items-center justify-between p-2 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <Toggle 
                enabled={isExpInstallment} 
                onChange={(val) => { setIsExpInstallment(val); if(val) setIsExpRecurrent(false); }} 
                activeColorClass="bg-indigo-500" 
                />
            </div>
            {!isExpInstallment && (
                <div className="flex items-center justify-between p-2 bg-slate-50 rounded-xl border border-slate-100 animate-in fade-in duration-300">
                    <span className="text-[10px] font-bold text-slate-600 uppercase">RECORRENTE</span>
                    <Toggle 
                    enabled={isExpRecurrent} 
                    onChange={setIsExpRecurrent} 
          ...
          {isExpInstallment ? (
            <div className="space-y-4 animate-in slide-in-from-top-2">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="totalValue" type="number" step="0.01" required className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none uppercase" placeholder="0,00" />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Parcelas</label>
                        <input name="installmentsCount" type="number" required min="2" max="120" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none uppercase" placeholder="12" />
                    </div>
                </div>
                <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Data Inicial</label>
                    <input name="startDate" type="date" required defaultValue={todayStr} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none uppercase" />
                </div>
                <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Frequência</label>
                    <select name="freq" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none uppercase">
                        <option value={Frequency.WEEKLY}>SEMANAL</option>
            ...
            <div className="space-y-4 animate-in slide-in-from-top-2">
                <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Valor</label>
                    <input name="value" type="number" step="0.01" required className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none uppercase" placeholder="0,00" />
                </div>
                <div>
                    <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Vencimento</label>
                    <input name="dueDate" type="date" required defaultValue={todayStr} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none uppercase" />
                </div>
                {isExpRecurrent && (
                    <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2">
                        <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Frequência</label>
                            <select name="freq" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none uppercase">
                                <option value={Frequency.DAILY}>DIÁRIO</option>
                                <option value={Frequency.WEEKLY}>SEMANAL</option>
                                <option value={Frequency.MONTHLY} selected>MENSAL</option>
                                <option value={Frequency.YEARLY}>ANUAL</option>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Repetições</label>
                            <input name="reps" type="number" defaultValue="12" min="2" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none uppercase" />
                        </div>
                    </div>
                )}
            </div>
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-1">
              ...
              <button 
                type="button" 
                onClick={() => setIsAddingExpCategory(!isAddingExpCategory)}
                className="text-rose-600 hover:text-rose-800 font-bold text-lg"
              >
                {isAddingExpCategory ? '✕' : '+'}
              </button>
            </div>
            ...
            {isAddingExpCategory ? (
              <div className="flex gap-2 animate-in slide-in-from-top-2">
                <input 
                  autoFocus
                  className="flex-1 bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none uppercase" 
                  placeholder="NOVA CATEGORIA"
                ...
                <button 
                  type="button" 
                  onClick={handleAddExpCategory}
                  className="bg-rose-600 text-white px-4 rounded-xl font-bold hover:bg-rose-700"
                >
                  OK
                </button>
              </div>
            ) : (
              <select name="category" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-rose-500 outline-none uppercase">
          ...
          <button type="submit" className={`w-full p-3 rounded-xl font-bold shadow-lg transition-all uppercase ${isExpInstallment ? 'bg-indigo-600 shadow-indigo-200 hover:bg-indigo-700' : 'bg-rose-600 shadow-rose-200 hover:bg-rose-700'} text-white`}>
            {isExpInstallment ? 'Gerar Parcelas' : 'Salvar Despesa'}
          </button>
        </form>
      </Modal>
      ...
      <Modal isOpen={showDebtForm} onClose={() => setShowDebtForm(false)} title="NOVO PARCELAMENTO">
        <form onSubmit={(e) => {
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Descrição</label>
            <input name="desc" type="text" required maxLength={30} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none uppercase" placeholder="Ex: GELADEIRA" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Valor Total</label>
              <input name="total" type="number" step="0.01" required className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none uppercase" placeholder="0,00" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Parcelas</label>
              <input name="count" type="number" required min="2" max="120" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none uppercase" placeholder="12" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Data Inicial</label>
            <input name="start" type="date" required defaultValue={todayStr} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none uppercase" />
          </div>
          <button type="submit" className="w-full bg-indigo-600 text-white p-3 rounded-xl font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all uppercase">Gerar</button>
        </form>
      </Modal>
